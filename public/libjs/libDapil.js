...
$("#tingkat_pemilihan").on('change', function() {
   $("#prop_dapil").val('').trigger('change');
   //loadMode = 'add';
        if($(this).val() == '1'){
            $("#kecamatandapil").empty();
            $(".kecamatanDapil").addClass('d-none');

        }
        else{
            $(".kecamatanDapil").removeClass('d-none');
        }
});
    ...
    $("#kab_dapil").select2({
         ...
         dropdownParent: $('#modal_data_dapil'),
         theme: 'bootstrap-5'
        });
        ...
        success: function(resp)
        {
            if (resp.status) {
               for(var prop in resp.data) {
                ...
                if(resp.detail !== false){

                    for(var p in resp.detail.data.prop){
                        $("#"+formID+" [name=prop_dapil]").val(resp.detail.data.prop.kode);
                    }


                    reSelectDapil(resp.detail, formID);
                }
...
function reSelectDapil(detail, formID){
    $("#"+formID+" [name=prop_dapil]").val(detail.data.prop.kode);
     let url =  currentClass+'selectKabupaten/'+detail.data.prop.kode;
    $('#kab_dapil').load(url,function() {
          //$('.SelectKabDapil').select2({ data:detail.data.kab.kode });
          //$(".SelectKabDapil option:not(:first)").prop("selected","selected");
        $(this).val(detail.data.kab.kode);
    });
     var settings4 = {
                    "groupDataArray"    : detail.data.kec,
                    "groupItemName"     : "groupName",
                    "groupArrayName"    : "groupData",
                    "itemName"          : "nama",
                    "valueName"         : "kode",
