...
$(document).ready(function () {
    ...
    $('#filter_caleg_id').load(currentClass + 'select_filter_caleg', function () {
        /* Act on the event */
    });
    $('#filter_dapil_id').load(currentClass + 'select_filter_dapil', function () {
        /* Act on the event */
    });

    $('#filter_propinsi').load(currentClass + 'select_filter_prop', function () {
        /* Act on the event */
    });
    $('#filter_propinsi').on('change', function () {
        $("#filter_kecamatan option:not(:first), #filter_kelurahan option:not(:first)").remove();
        $('#filter_kabupaten').load(currentClass + 'select_filter_kabupaten/' + $(this).val(), function () {
            /* Act on the event */
        });
    });
    ...
    $('#filter_kabupaten').on('change', function () {
        ...
        $('#filter_kecamatan').load(currentClass + 'select_filter_kecamatan/' + $(this).val(), function () {
            /* Act on the event */
        });
    });

    $('#filter_kecamatan').on('change', function () {
        $('#filter_kelurahan').load(currentClass + 'select_filter_kelurahan/' + $(this).val(), function () {
            /* Act on the event */
        });
    });

    $('#filter_relawan_id').load(currentClass + 'select_filter_relawan', function () {
        /* Act on the event */
    });
...
function dashBoxData() {
    let url = currentClass + '/dashbox';
    let anggotaWanita = $(".anggotaWanita");
    let anggotaPria = $(".anggotaPria");
    ...
    $.ajax({
        ...
        success: function (res) {
            console.log(res);
            if (res.status) {
                anggotaWanita.text(res.anggota_wanita);
                anggotaPria.text(res.anggota_pria);
                totalAnggota.text(res.total_anggota);
...
function loadDataTable(el, pageLength = 10) {
...
function action_btn() {
    $('.addNilaiTes').click(function () {
        console.log('addNilaiTes klik');
        let url = currentClass + 'add_nilai_tes';
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'JSON',
            data: { jadwal_seleksi_id: dataValue },
