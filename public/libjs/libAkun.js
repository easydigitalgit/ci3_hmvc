...
var save_method;
var urlSave  = ''; 
var tblakun = $('#table_data_akun')



$(document).ready(function() {
   
    loadDataTable(tblakun);
   
    $("#check-all").click(function () {
        $(".data-check").prop('checked', $(this).prop('checked'));
    });

    $('#level_user_id').load(currentClass+'select_user_level', function() {
        /* Act on the event */
    });

    $('#level_user_id').on('change',  function(event) {
        event.preventDefault();
        let val = $(this).val();
        if(val == '2'){
            $('.namaCaleg').removeClass('d-none');
        }else {
            $('.namaCaleg').addClass('d-none');
        }
    });

    $('#modal_data_akun').on('hidden.bs.modal', function (e) {
        ...
        success: function(resp)
        {
            if (resp.status) {
                for(var prop in resp.data) {
                    ...
                    if(prop == 'foto'){
                         $("#foto_akun").attr("src",base_url+'AppDoc/foto_akun/'+resp.data[prop]);
                    }
                    else{
