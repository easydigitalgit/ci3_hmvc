        ...
        success: function(resp)
        {
            if (resp.status) {
                ...
                for(var prop in resp.data) {
                    ...
                    if (prop == 'scan_payment' || prop == 'scan_akte') {
                        let kodeUnit =    resp.data['kode_unit'];
                        let file = base_url +'AppDoc/'+kodeUnit.toLowerCase()+'/'+ resp.data[prop];

                        let element = '<a class="image-popup-vertical-fit" href="'+file+'" title="Image Lampiran"> <img class="img-fluid" alt="" src="'+file+'" width="145"> </a>';


                        prop == 'scan_akte' ? $(".imgScanAkte").append(element) :  $(".imgScanPayment").append(element);
                ...
                reSelectJenjangKelas(resp.data['tingkat_id'],resp.data['jenjang_kelas_id']);           
                console.log('jenjang_kelas_id = '+resp.data['jenjang_kelas_id'] );
...
function reSelectJenjangKelas(unitID, jenjangKelas){
    //$(".selectJenjangKelas").load(currentClass+'/select_jenjang_kelas/'+unitID);
    $(".selectTingkat").val(unitID);
    $(".selectJenjangKelas").load(currentClass+'/select_jenjang_kelas/'+unitID, function() {
        $(".selectJenjangKelas").val(jenjangKelas)
    });
    console.log('call reSelectJenjangKelas = '+unitID+ '-' + jenjangKelas);
}
